...
const getTooltipStyle = (rect) => {
  ...
  const tooltipW = Math.min(280, vw - 28);
  const tooltipH = 230;
  const gap = 14;

  if (!rect) {
    return { position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: tooltipW };
  }

  const goAbove = rect.top > vh * 0.55 || rect.bottom + tooltipH + gap > vh;
  let top = goAbove ? Math.max(gap, rect.top - tooltipH - gap) : rect.bottom + gap;
...
const CustomTutorial = ({ steps, storageKey, delay = 900 }) => {
  ...
  return (
    <>
      <div className="fixed inset-0 z-[9000]" style={{ background: "rgba(0,0,0,0.55)", pointerEvents: "none" }} />
      {rect && (
        <div
          ...
          style={{
            ...
            border: "2.5px solid #3b82f6", pointerEvents: "none",
          }}
        />
      )}
      <div className="z-[9002] bg-white rounded-2xl shadow-2xl p-5" style={getTooltipStyle(rect)}>
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <div className="flex gap-2">
            {!isFirst && (
              <button onClick={back} className="text-xs bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg">← Back</button>
            )}
            {isLast ? (
              <button onClick={finish} className="text-xs bg-blue-500 hover:bg-blue-600 text-white px-4 py-1.5 rounded-lg font-semibold">Done ✓</button>
            ) : (
              <button onClick={next} className="text-xs bg-blue-500 hover:bg-blue-600 text-white px-4 py-1.5 rounded-lg font-semibold">Next →</button>
            )}
          </div>
        </div>
      </div>
